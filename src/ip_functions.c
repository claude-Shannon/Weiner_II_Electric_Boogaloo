...
void baby_beat(void) //just mom
{
	...
	u32 baby_out, in_left, prev_left, out_left;
...
void both_beat(void) //both heart beats
{
	...
	u32 baby_out, mom_out, in_left, prev_left, out_left;
	...
	//		out_right = temp + in_right;
			...
			/* Output corrupted audio to the codec */
			Xil_Out32(I2S_DATA_TX_L_REG, mom_out+baby_out);
			Xil_Out32(I2S_DATA_TX_R_REG, mom_out+baby_out);
...
void filter(void) //just mom
{
	...
	static u32 baby_address = 0;
	static u32 mom_address = 0;
	u32 baby_out, mom_out, in_left, prev_left, out_left, filter_out;
	...
	if(in_left != prev_left)
		{
			...
			XLms_hw_Set_signal_noise_V(&lms, mom_out+baby_out);
			filter_out = XLms_hw_Get_output_V(&lms);
